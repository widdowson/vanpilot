...
| 1.3 | Default teal — DHU light mode | `dhu/1.3_visual_dhu_light.png` | HAS | Full DHU screenshot with tab bar, AA chrome. Surface renders dark gray (NavigationTemplate surface not initialized — see notes). Status bar masked. |
...
| 2.2 | Populated conversation (mock data) | `dhu/2.2_lead_agent_populated.png` | HAS | 3 mock messages from `createWithMockData()`. Real DHU capture, status bar masked. |
...
| # | State | File | Status | Notes |
|---|-------|------|--------|-------|
| 3.1 | Single sub-agent — "Researcher" | `dhu/3.1_researcher_messages.png` | HAS | 2 mock messages for "researcher" agent. Real DHU capture, status bar masked. |
| 3.2 | Second sub-agent — "Coder" | `dhu/3.2_coder_messages.png` | HAS | 1 mock message for "coder" agent. Real DHU capture, status bar masked. |
...
| # | State | File | Status | Notes |
|---|-------|------|--------|-------|
| 4.1 | Visual tab selected (default) | `dhu/4.1_visual_tab_selected.png` | HAS | Visual tab active, others inactive. Real DHU capture, status bar masked. |
| 4.2 | Lead Agent tab selected | `dhu/4.2_lead_agent_tab_selected.png` | HAS | Lead Agent tab active, content = message list. Real DHU capture, status bar masked. |
| 4.3 | Sub-agent tab selected | `dhu/4.3_sub_agent_tab_selected.png` | HAS | Researcher tab active. Real DHU capture, status bar masked. |
| 4.4 | All 4 tabs visible | `dhu/4.4_all_four_tabs.png` | HAS | Visual + Lead + 2 sub-agents at max capacity. Real DHU capture, status bar masked. |
...
3. **P2 — Advanced** (nice to have):
   - Connection indicators (blocked on UI wiring, #124)
